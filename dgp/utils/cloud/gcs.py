...
def sync_dir_gcs(source, target, file_ext=None, verbose=True):
    ...
    """
    print("Inside syncdir")
    if verbose:
        logging.getLogger().setLevel(logging.DEBUG)

    assert source.startswith('gs://') or target.startswith('gs://')
    additional_flags = '-r'
    if file_ext is not None:
        additional_flags = f'--exclude=* --include=*{file_ext}'

    command_str = "gsutil -m rsync {} {} {}".format(
        additional_flags, source, target
    )
    print("Sync Command generated --> "+ command_str)
...
def get_string_from_gcs_file(bucket_name, url):
    ...
    print("Inside get_string_from_gcs_file")
    storage_client = storage.Client()
    bucket = storage_client.bucket(bucket_name)
    print('Fetching{} '+ url+ ' From Bucket :'+ bucket_name)
    blob = bucket.blob(url)

    content = blob.download_as_string()  # Download the blob content
    return content.decode('utf-8')  # Decode as UTF-8
...
def open_remote_json(s3_path):
    ...
    return json.loads(get_string_from_gcs_file(bucket, rel_path))
